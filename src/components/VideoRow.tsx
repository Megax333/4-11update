...
interface VideoRowProps {
  ...
  useFullSizedCovers?: boolean;
}

const VideoRow: React.FC<VideoRowProps> = ({ title, items, showNavigationArrows = true, useFullSizedCovers = false }) => {
  ...
  // Calculate maximum index based on total items and items per view
  ...
  const navigateToSeries = (id: string) => {
    setActiveMovie(items.find(movie => movie.id === id) || null);
  };
      ...
      >
        ...
        >
          {items.map((item) => (
            <div 
              key={item.id} 
              className={`group relative flex-shrink-0 w-[calc(100%/6-16px)] mr-4`}
            ...
            >
              <div className="rounded-xl overflow-hidden">
                <img
                  src={item.thumbnail}
                  alt=""
                  className={`w-full ${useFullSizedCovers ? 'aspect-[2/3]' : 'aspect-[16/9]'} object-cover`}
                />
              </div>
